  ...
  <div class="tw-tabs-section">
    <div class="tw-container">
      ...
      <div class="tw-tabs-nav">
        ...
        {% for block in section.blocks %}
          {% if block.type == 'tab' %}
            ...
            <button class="tw-tab-btn {% if tab_index == 1 %}is-active{% endif %}" data-target="tab-{{ block.id }}">
      ...
      <div class="tw-tabs-content-area">
        {% assign tab_index = 0 %}
        {% assign in_tab = false %}
        {% assign current_tab_type = '' %}
        
        {% for block in section.blocks %}
          {% if block.type == 'tab' %}
            
            {% if in_tab %}
              {% if current_tab_type == 'color_overviews' %}</div>{% endif %}
              </div>
            {% endif %}
            
            {% assign tab_index = tab_index | plus: 1 %}
            {% assign in_tab = true %}
            {% assign current_tab_type = block.settings.tab_content_type %}
            
            <div class="tw-tab-content {% if tab_index == 1 %}is-active{% endif %}" id="tab-{{ block.id }}" {{ block.shopify_attributes }}>
              {% if current_tab_type == 'color_overviews' %}
          ...
          {% elsif block.type == 'color_image' %}
            {% if in_tab and current_tab_type == 'color_overviews' %}
