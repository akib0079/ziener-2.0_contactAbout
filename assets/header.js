...
class HeaderComponent extends Component {
  ...
  #observeStickyPosition = (alwaysSticky = true) => {
    ...
    this.#intersectionObserver = new IntersectionObserver(([entry]) => {
      ...
      document.body.classList.toggle('sticky-header', this.dataset.stickyState == 'active')
    }, config);
  ...
  connectedCallback() {
    ...
    this.querySelectorAll('.header-menu__inner .menu-list__link:not(.megamenu-link)').forEach(a => {
      a.addEventListener('mouseenter', () => {
        document.querySelector(`header-megamenu:not([id="${this.id}"]).active`)?.forceClose();
      })
    })
    this.initSearch();
  }
  ...
  initSearch(){
    ...
    if(!popup) return;
    this.querySelectorAll('.header__search-form').forEach(ele => {
      ele.addEventListener('click', e => {
        e.preventDefault();
        document.body.classList.add('search-active');
        setTimeout(() => {
          popup.querySelector('.header__search-form .header__search-field').focus();
        }, 210)
        
      })
    })
    document.querySelectorAll('.header__search-popup-close, .header__search-bg-overlay').forEach(ele => {
      ele.addEventListener('click', e => {
        e.preventDefault();
